...
from gcn.utils import load_data, accuracy, loss_param
import syft as sy
...
def federated_train(epoch):
    ...
    if (epoch + 1) % args.E == 0:
        ...
        for param_i in range(len(params[0])):
            ...
            for remote_index in range(len(compute_nodes)):
                spdz_params.append(
                    loss_param(params[remote_index][param_i]).fix_precision().share(bob, alice, crypto_provider=james))
...
def plot():
    ...
    plt.show()
    # plt.savefig(r'C:\Users\lzl_z\Desktop\Fed_GCN_Experiment_three\image\20_' + str(args.zubie) + '.png')
...
def final_test():
    ...
    df_test = pd.DataFrame(data=[[acc_test_local.item(), loss_test_local.item()],
                                 [acc_test_bob_fed.item(), loss_test_bob_fed.item()]],
                           columns=['acc', 'loss'], index=['local', 'fed'])
    with open(r'C:\Users\lzl_z\Desktop\Fed_GCN_Experiment_three\citeseer-results\testing\_20_'+str(args.zubie)+'.pkl', 'wb') as f5:
        pkl.dump(df_test, f5)
    f5.close()
    print(df_test)
...
val_info_dict = {'local_val_acc': accuracy_val_list[2], 'fed_val_acc': accuracy_val_list[3],
                 'local_val_loss': loss_val_list[2], 'fed_val_loss': loss_val_list[3]}
with open(r'C:\Users\lzl_z\Desktop\Fed_GCN_Experiment_three\citeseer-results\validation\_20_'+str(args.zubie)+'.pkl', 'wb') as f:
    pkl.dump(val_info_dict, f)
f.close()

# model saving
# with open(r'C:\Users\lzl_z\Desktop\Fed_GCN_Experiment_two\model\bob_alone_E' + str(args.E) + '_' + str(
#         args.zubie) + '.pkl', 'wb') as f1:
#     torch.save(bob_alone_model, f1)
# f1.close()
# with open(r'C:\Users\lzl_z\Desktop\Fed_GCN_Experiment_two\model\alice_alone_E' + str(args.E) + '_' + str(
#         args.zubie) + '.pkl', 'wb') as f2:
#     torch.save(alice_alone_model, f2)
# f2.close()
# with open(r'C:\Users\lzl_z\Desktop\Fed_GCN_Experiment_three\model\local_20_' + str(args.zubie) + '.pkl',
#           'wb') as f3:
#     torch.save(local_model, f3)
# f3.close()
# with open(r'C:\Users\lzl_z\Desktop\Fed_GCN_Experiment_three\model\fed_20_' + str(args.zubie) + '.pkl',
#           'wb') as f4:
